...
public partial class SpriteAnimator : EditorWindow 
{
	...
	class AnimEvent
	{
		...
		// Our custom data
		public bool m_sendUpwards = false; // Whether events will be sent upwards (requires PowerAnimEventHandler or PowerSpriteAnim component)
		public bool m_usePrefix = false; // Whether "Anim" is prefixed to events
	...
	void LayoutFrameSprite(Rect rect, Sprite sprite, float scale, Vector2 offset, bool useTextureRect, bool clipToRect)
	{
		if ( rect.width <= 10 || rect.height <= 10 )
			return;

		// If the sprite is packed we render it in a less efficient but also less broken way
		if ( sprite.packed )
			LayoutFrameSpriteRendered(rect,sprite,scale,offset,useTextureRect,clipToRect);
		else
			LayoutFrameSpriteTexture(rect,sprite,scale,offset,useTextureRect,clipToRect);

	}

	// This layout just draws the sprite using gui tools
	void LayoutFrameSpriteTexture(Rect rect, Sprite sprite, float scale, Vector2 offset, bool useTextureRect, bool clipToRect)
	{
	...
	public void LayoutFrameSpriteRendered(Rect rect, Sprite sprite, float scale, Vector2 offset, bool useTextureRect, bool clipToRect)
	{
		...
		// Offset from pivot so that sprite is centered correcctly (ignoring pivot point, for now at least)
		Vector2 pivotOffset = -((sprite.rect.size*0.5f)-(sprite.pivot));
	...
	void CreateCurveBinding()
	{
		...
		m_curveBinding.path = ""; // Potentially it'd be nice to be able to have the animator on the root, but the sprite as a child. Not sure how to make this simple though
