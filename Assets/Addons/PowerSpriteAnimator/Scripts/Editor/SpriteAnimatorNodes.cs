...
namespace PowerTools
{
	...
	static readonly int KEY_MODE_CONSTANT_RIGHT_LEFT = ((KEY_MODE_CONSTANT << 1) & -25) | (KEY_MODE_CONSTANT << 3 );
		...
		void LayoutNode(Rect rect, int nodeId, AnimFrame currentFrame)
		{
			...
			if (string.IsNullOrEmpty(m_defaultNodeNames[nodeId]) == false)
			...
			if (m_selectedNode == nodeId)
			{
				...
				{
					// Draw crosshair
					const float crosshairLength = 20;
		...
		void LayoutBottomBarNodeData(Rect rect)
		{
			...
			xOffset += width;
			labelContent = new GUIContent("Offset:  x");
			...
			pos.x = EditorGUI.FloatField(new Rect(xOffset, 2, width, rect.height - 3), pos.x, EditorStyles.miniTextField);
			...
			pos.y = EditorGUI.FloatField(new Rect(xOffset, 2, width, rect.height - 3), pos.y, EditorStyles.miniTextField);
			...
			// Display Name
			xOffset += width + 10;
			labelContent = new GUIContent("Display Name: ");
			width = EditorStyles.miniLabel.CalcSize(labelContent).x;
		...
		// Sets the x offset of a node on a particular anim frame, adding it if it didn't exist.
		...
		{
			if (!angle && Mathf.Approximately(value, lastValue))
			{
		...
		void CreateStaticBindings()
		{
			if (NODE_BINDINGS_X == null || NODE_BINDINGS_Y == null || NODE_BINDINGS_ANGLE == null)
			{
				...
				for (int nodeId = 0; nodeId < SpriteAnimNodes.NUM_NODES; ++nodeId)
				{
					NODE_BINDINGS_X[nodeId] = new EditorCurveBinding() { type = TYPE_SPRITEANIMNODES, propertyName = string.Format(PROPERTY_NODE_X, nodeId) };
					NODE_BINDINGS_Y[nodeId] = new EditorCurveBinding() { type = TYPE_SPRITEANIMNODES, propertyName = string.Format(PROPERTY_NODE_Y, nodeId) };
					NODE_BINDINGS_ANGLE[nodeId] = new EditorCurveBinding() { type = TYPE_SPRITEANIMNODES, propertyName = string.Format(PROPERTY_NODE_ANGLE, nodeId) };
				}
			}
		}
