...
namespace PowerTools
{
	public partial class SpriteAnimator
	{
		...
		void LayoutEventVisuals(AnimEvent animEvent, bool selected, Rect eventRect, string labelText, Rect labelRect)
		{
			...
			GUI.Label(new Rect(labelRect) { yMin = labelRect.yMin - 4 }, labelText, new GUIStyle(Styles.TIMELINE_EVENT_TEXT) { normal = { textColor = eventColor } });
		...
		void LayoutTimelineSprite(Rect rect, Sprite sprite)
		{
			...
			{
				float widthScaled = rect.width / sprite.textureRect.width;
				float heightScaled = rect.height / sprite.textureRect.height;
